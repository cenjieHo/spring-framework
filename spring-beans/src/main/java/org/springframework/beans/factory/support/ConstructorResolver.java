...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		if (constructorToUse == null) {
			...
			boolean autowiring = (chosenCtors != null ||	// 如果 chosenCtors 传入
			...
			if (candidates == null) {
				Class<?> beanClass = mbd.getBeanClass();			// 获取到bean的class对象
				try {
					candidates = (mbd.isNonPublicAccessAllowed() ?	// 查看是否运行访问非public的构造函数和方法
							beanClass.getDeclaredConstructors() : 	// 如果允许，则使用getDeclaredConstructor()方法返回所有构造器
							beanClass.getConstructors());			// 否则，是用getConstructors()方法返回public的构造器
				}
				catch (Throwable ex) {
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		if (factoryBeanName != null) {
			if (factoryBeanName.equals(beanName)) {	// 如果工厂名等于bean名，抛出 BeanDefinitionStoreException 异常
				throw new BeanDefinitionStoreException(mbd.getResourceDescription(), beanName,
						"factory-bean reference points back to the same bean definition");
			}
			// 获取工厂实例
			factoryBean = this.beanFactory.getBean(factoryBeanName);
			if (mbd.isSingleton() && this.beanFactory.containsSingleton(beanName)) {	// 如果是单例模式并且已经创建过了这个bean，那么就抛出异常
				throw new ImplicitlyAppearedSingletonException();
			}
			factoryClass = factoryBean.getClass();	// 获取该工厂 bean 的 class
			isStatic = false;						// 标记为非静态工厂
		}
		else {
			// todo：why？？？
			// 工厂名为空，则其可能是一个静态工厂
		...
		if (explicitArgs != null) {
			...
			// 缓存中的值可能是原始值也有可能是最终值
			if (argsToResolve != null) {
				argsToUse = resolvePreparedArguments(beanName, mbd, bw, factoryMethodToUse, argsToResolve);	// TODO：暂未研究
			}
		}
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			for (Method candidate : candidates) {
				...
				if (paramTypes.length >= minNrOfArgs) {
					...
					// isLenientConstructorResolution 判断解析构造函数的时候是以宽松模式还是严格模式
					// 严格模式：解析构造函数的时候，必须所有的都需要匹配，否则抛出异常
		...
		try {
			...
			// 上面及其复杂的代码总结下来就是：确定工厂对象，获取构造函数和构造参数，最后调用 instantiate 方法创建 Bean 实例并设置到 bw 中
			bw.setBeanInstance(beanInstance);
			return bw;
		}
		catch (Throwable ex) {
